...
//      super.configure(http); // 이 코드 삭제하면 기존 시큐리티가 가진 모든 기능 비활성화
        ...
        http.authorizeRequests()
                .antMatchers("/text", "/partnership").authenticated() // 이 주소로 시작되면 인증이 필요
                .anyRequest().permitAll() // 그게 아닌 모든 주소는 인증 필요 없음
                .and()
                .formLogin()
                .loginPage("/test/login") // 인증필요한 주소로 접속하면 이 주소로 이동시킴
                .loginProcessingUrl("/test/login") // 스프링 시큐리티가 로그인 자동 진행 POST방식으로 로그인 진행
